  ...
  <header class="sticky top-0 z-50 bg-white/80 dark:bg-stone-900/80 backdrop-blur border-b border-stone-200 dark:border-stone-700 shrink-0">
    <div class="px-4 py-2.5 flex items-center gap-4">
      ...
      <div class="flex-1 flex items-center min-w-0 w-full">
        <!-- Read-only mode -->
        <div v-if="!editing" class="flex items-center gap-1.5 justify-between bg-stone-100 pr-2 pl-3 rounded-full h-7 min-w-0 w-full">
          <span class="text-xs font-mono text-stone-400 dark:text-stone-500 truncate">{{ route }}</span>
          <button
            class="w-5 h-5 flex items-center justify-center rounded text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors cursor-pointer shrink-0"
            title="Edit route"
            @click="startEdit"
          >
        ...
        <div v-else class="flex items-center gap-1.5 flex-1 min-w-0">
          <input
            ...
            spellcheck="false"
            autofocus
            class="flex-1 px-2 py-1 text-xs font-mono bg-white dark:bg-stone-800 border border-stone-300 dark:border-stone-600 rounded-md text-stone-700 dark:text-stone-200 outline-none focus:border-blue-400 dark:focus:border-blue-500 min-w-0"
            :disabled="navigating"
            @keydown="onKeydown"
            @blur="editing = false"
          />
          <button
            :disabled="navigating"
            class="px-2 py-1 text-xs font-medium text-white bg-stone-700 dark:bg-stone-600 rounded-md hover:bg-stone-800 dark:hover:bg-stone-500 disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
      ...
      <div class="flex items-center gap-3 shrink-0">
        ...
        <a
          ...
          target="_blank"
          class="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-stone-500 dark:text-stone-400 bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-full hover:border-stone-300 dark:hover:border-stone-600 hover:text-stone-700 dark:hover:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-700 transition-all no-underline"
        >
